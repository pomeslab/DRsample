...
void showUsage(const char *c){
	fprintf(stderr,"Usage: %s <script file> <database text file> <sequence numbers to skip> <max sequence number to use> <data to use> <JOB to use> > wham.input\n",c);
	...
	fprintf(stderr,"       - <JOB to use> : 0 for all, 1 for only the first listed JOB, 2 for the second listed job...\n");
}
...
int main(int argn, char *args[]){
	...
	sscanf(args[5],"%d",&dtu);
	sscanf(args[6],"%d",&jtu);
	jtu--; //read it in as 1->N but my index runs 0->N-1
	...
	while(fgets(linein,1000,f)!=NULL){
		...
		if(sscanf(linein,"%*s %f",&jobs[njobs])!=1){
		...
		if(sscanf(linein,"%*s %*f %*d %*d %lf",&fc[njobs])!=1){
			//May still want to use the extraction routine for a temperature reaction coordinate
			fc[njobs]=0.0;
		}
	...
	while(fgets(linein,1000,f)!=NULL){
		...
		if(linein[0]=='A'&&linein[1]=='d'&&linein[2]=='d'){
			...
			if(s>nskip && (nmax<0 || s<nmax) && (jtu<0||jtu==n)){
				fprintf(g[n],"%f %d %d\n",data,r,s);
			}
			continue;
		}
	...
	for(i=0;i<njobs;i++){
		if(jtu<0||jtu==i){
			sprintf(command,"cat ./%s|sort -n -s -k 3,3 | awk '{print $3,$1,$2}' > sorting.tmp",name[i]);
			system(command);
