...
public:
	...
	force_database_class(const char *title, unsigned char verbose_p)
	{
		...
		if(verbose) fprintf(stderr,"The size of the database file is: %lld\n",filesize);

		
		if(filesize>=(long long)sizeof(header))
		{
			lseek(fd,0,SEEK_SET);
 ...
 */
	...
	unsigned char read_record(long long int record_number)
	{
		header_existence_check();
		
		if(record_number>=header.Nrecords) return(0);

		long long int record_position=(long long int)sizeof(header)+(long long int)record_number*record_size;
		
		lseek(fd,record_position,SEEK_SET);
		
		if(read(fd,record,record_size)!=record_size)
		{
			fprintf(stderr,"Error: read of record failed (read_record)\n");
			exit(1);
		}
		return(1);
	}
	
	unsigned char read_record_to_given_memory(long long int record_number, struct record_struct *rec)
	{
		long long int check, record_position;

		header_existence_check();
		
		if(record_number>=header.Nrecords) return(0);

		record_position=(long long int)sizeof(header)+(long long int)record_number*record_size;
		
		check=(long long int)lseek(fd,record_position,SEEK_SET);
		if(check!=record_position){
			fprintf(stderr,"Error: lseek failed (read_record_to_given_memory) record %d at position %d lseek returned %d\n",record_number,record_position,check);
			perror("ERROR: ");
		}
	
		check=(long long int)read(fd,rec,record_size);	
		if(check!=record_size)
		{
			fprintf(stderr,"Error: read of record failed (read_record_to_given_memory) record %d at position %d read only size %d\n",record_number,record_position,check);
			perror("ERROR: ");
			exit(1);
			//return(0);
		}
