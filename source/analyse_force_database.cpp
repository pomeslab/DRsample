...
#define MAX_SEQUENCE_NUMBER 500000 
#define NUMVALUES 10  //number of values output to define the y-axis
//#define MAX_DATABASE_SIZE 4000000000  // this can prevent overflows of unsigned long int
#define MAX_DATABASE_SIZE 4000000000000
...
struct plotinfo_struct{
	...
	char ytitle[500];
	char title[500];
	long xa,xb,xc; //variables for looping over xgrid
	long ya,yb,yc; //variables for looping over ygrid
...
struct sampleDensity_struct{
	...
	float min;
	float max;
	unsigned long Ncol;      //number of histogram bins
	float binWidth;
};
#define EMPTY_SAMPLEDENSITY {(unsigned long **)NULL,0.0,0.0,0,0.0}
...
#define EMPTY_PMF {(float *)NULL,(unsigned long *)NULL,0.0,0.0,0,0.0}

struct exact_struct{
	float *e;		//the exact pmf value
	float *b;		//the bin
	unsigned long n;		//num bins
};
...
struct analysis_option_struct{
	char title[3];
	long sequence_number_limit;
	long writeDatabase;
	long writeDatabaseFull;
	char writeDatabaseName[100];
	long userAskedForFitting;
	long verbose;
	long useCancellation;
	long additionalDataWithSampling;
	long histNcol;
	long discardOutside;
	long useExact;
	float equilFraction;
	long justwriteDatabase;
};
...
long parseCommandLine(long argc,char * const argv[], struct analysis_option_struct *opt);
long checkOptions(struct analysis_option_struct *opt, const struct script_struct *script);
//Main
...
void print_postscript_trajectory(long printSelection,long circularFlag, long ignorerounds, const struct pageinfo_struct *pageinfo, long plotRCinstead, struct detailedBalance_struct *detbal, const struct database_struct *db, const struct graph_struct *graph, const struct analysis_option_struct *opt, const struct script_struct *script, const struct stats_struct *stats, long storecode);
void print_postscript_sequenceDensity(const struct pageinfo_struct *pageinfo, unsigned long * const *sequenceDensity, const struct script_struct *script, const struct graph_struct *graph);
...
void print_postscript_samplingOverlap(long whichData, const struct pageinfo_struct *pageinfo, const struct script_struct *script, struct database_struct *db, const struct nominal_struct *nominal, const struct graph_struct *graph, const struct analysis_option_struct *opt, const struct stats_struct *stats);
...
void quicksort_database(struct database_struct *db, long left, long right);
struct record_struct* rec(unsigned long record_number,const struct database_struct *db);
long read_in_database(const struct analysis_option_struct *opt, const struct script_struct *script, struct database_struct *db, const struct nominal_struct *nominal);
char get_next_force(float w[2], float force[2], float *rc, long whichRC, unsigned long *time, long *replica_number, unsigned short *sequence_number, const struct database_struct *db);
void get_data_statistics(struct stats_struct *stats, const struct database_struct *db);
void condense_forces(unsigned long N_values_to_average, struct nominal_struct *nominal, struct graph_struct *graph, const struct script_struct *script, const analysis_option_struct *opt, struct stats_struct *stats, const struct database_struct *db);
exact_struct * getExactFromFile(const char *title, exact_struct *exact, const struct script_struct *script);
long getCancellationFromLog(const char *title, float *cancel, const struct script_struct *script);
long find_bin_from_w(double w, long l, const struct nominal_struct *nominal, const struct script_struct *script);
long round_up(float x);
long round_down(float x);
void rot_trans_regular(void);
char *get_RGB_colour(long colour_num);
void setup_regular(unsigned char *current_page);
void calcSequenceDensity(unsigned long **sequenceDensity, const struct database_struct *db, const struct nominal_struct *nominal, const struct script_struct *script, const analysis_option_struct *opt, const struct stats_struct *stats);
void getMinMaxSampleDensity(float *min, float *max, unsigned long whichData, unsigned long whichReplica, const struct database_struct *db, const struct nominal_struct *nominal, const struct script_struct *script);
long calcSampleDensity(struct sampleDensity_struct *sd, unsigned long whichData, const struct graph_struct *graph, const struct database_struct *db, const struct nominal_struct *nominal, const struct script_struct *script, const struct analysis_option_struct *opt, const struct stats_struct *stats);
long calcPMF(struct pmf_struct *pmf, unsigned long whichData, const struct graph_struct *graph, const struct database_struct *db, const struct nominal_struct *nominal, const struct script_struct *script, const struct analysis_option_struct *opt);
void setFont(unsigned long size);
long floatEqual(float i,float j);
void showFirstComboPageText(long px, long py);
void showSampleDensityNotAvailable(long px,long py,const struct script_struct *script,const struct analysis_option_struct *opt);
void showSecondComboPageText(long px,long py);
...
void showUsage(const char *c, const struct analysis_option_struct *opt){
	...
	printf("       -l [int] sequence-number-limit; negative indicates no limit (default = %ld)\n",opt->sequence_number_limit);
	printf("       -c [int] plot cancellation data from tt.log file (default = %ld)\n",opt->useCancellation);
	...
	printf("       -t [int] text-database-type (default = %ld)\n",opt->writeDatabaseFull);
	...
	printf("       -f [int] do-fitting-convergence-analysis (default = %ld)\n",opt->userAskedForFitting);
	printf("          ( =0) do not attempt\n");
	printf("          (!=0) do fitting (user beware)\n"); 
	printf("       -a [int0 or >=1] which additional_data holds your sampling value; zero indicates not to use (default = %ld)\n",opt->additionalDataWithSampling); 
	printf("       -h [int 0 or >=1] number of histogram bins for customizable plots; use zero for = Nreplicas (default = %ld)\n",opt->histNcol);
	printf("       -m [int] discard data outside JOB range for for customizable plots (default = %ld)\n",opt->discardOutside);
...
long checkOptions(struct analysis_option_struct *opt, const struct script_struct *script){
	if(opt->additionalDataWithSampling>0&&opt->additionalDataWithSampling>script->Nadditional_data){
		fprintf(stderr,"Error: Your run ony has %ld additional data columns, and you asked to use column %ld with the -a option on the command line\n",script->Nadditional_data,opt->additionalDataWithSampling);
		return 1;
	}
	if(opt->histNcol<0){
...
int main(long argc, char *argv[]){
	...
	unsigned long **sequenceDensity=(unsigned long **)NULL;
	...
	sampleDensity.Ncol=pmf.Ncol=(unsigned long)opt.histNcol;
	fprintf(stderr,"Configurable histograms will have %ld columns\n",opt.histNcol);
	...
	fprintf(stderr,"Maximum force points found to be %lu\n",stats.max_time);
	fprintf(stderr,"Maximum sequence number found to be %lu\n",stats.max_sequence_number);
	...
	sequenceDensity=(unsigned long **)malloc((script.Nreplicas+1)*sizeof(unsigned long *));
	if(sequenceDensity==NULL){
		fprintf(stderr,"Error: Unable to allocate memory for sequenceDensity\n");
		exit(1);
	}
	for(i=0; i<=script.Nreplicas; i++){
		sequenceDensity[i]=(unsigned long *)malloc((script.Nreplicas+1)*sizeof(unsigned long));
	...
	if(opt.additionalDataWithSampling>0){
		sampleDensity.b=(unsigned long **)malloc((script.Nreplicas+1)*sizeof(unsigned long *));
		if(sampleDensity.b==NULL){
			fprintf(stderr,"Error: Unable to allocate memory for sampleDensity.b\n");
			exit(1);
		}
		for(i=0; i<=script.Nreplicas; i++){
			sampleDensity.b[i]=(unsigned long *)malloc((sampleDensity.Ncol+1)*sizeof(unsigned long));
		...
		pmf.n=(unsigned long *)malloc((pmf.Ncol+1)*sizeof(unsigned long));
		if(pmf.n==NULL){
	...
	if(script.circular_replica_coordinate){
		...
		if(script.Nadditional_data>0){
			...
			for(long xq=0;xq<script.Nreplicas;xq++){
				setup_regular(&current_page);
...
void print_postscript_forceAverage(unsigned char ligand_number, const struct graph_struct *graph, const struct pageinfo_struct *pageinfo, const struct script_struct *script, const struct stats_struct *stats, const struct analysis_option_struct *opt){
	...
	for(i=plotinfo.xa;i<plotinfo.xb;i++){
		...
		printf("newpath\n");
		first=1;
		unsigned long max_force_points=0;
		for(j=0;j<N_FORCE_POINTS;j++)
			if(graph[i].weight_sum[j]>1.0e-5) max_force_points=j;

		for(j=(unsigned long)((float)max_force_points*opt->equilFraction);j<N_FORCE_POINTS;j++){
		//for(j=0;j<N_FORCE_POINTS;j++){
			force=graph[i].point[ligand_number][j];
			//printf("%% WEIGHT SUM: [i,j]=[%ld %lu]  %lf\n",i,j,graph[i].weight_sum[j]);
			if(graph[i].weight_sum[j]>=0.1){
				//fprintf(stderr,"FORCE: %lu: %f\n",j,force);
...
void print_postscript_trajectory(long printSelection,long circularFlag, long ignorerounds, const struct pageinfo_struct *pageinfo, long plotRCinstead, struct detailedBalance_struct *detbal, const struct database_struct *db, const struct graph_struct *graph, const struct analysis_option_struct *opt, const struct script_struct *script, const struct stats_struct *stats, long storecode){
	...
	static long dataflag=1; //ensures that this data is only printed the first time this routine is run
        static float storemax;
        static float storemin;
	long i,j,k,oldk,kmod,maxkmod,minkmod,didwrap;
	...
	long autodetectignore,autodetectnotyet,didmove;
	float old_position;
	...
	if(circularFlag>0){
		...
		for(i=0;i<script->Nreplicas;i++){
			...
			first=1;kmod=0;
			for(j=(unsigned long)((float)stats->max_sequence_number*opt->equilFraction);j<=stats->max_sequence_number;j++){
	...
	if(plotRCinstead){
		...
		plotinfo.xc=(long)(ceil)((float)(opt->histNcol)/(float)script->Nreplicas);
	...
	unsigned long productivity_ratio_count=0;
	if(script->coordinate_type==Temperature && plotRCinstead<0){
		printf(DASHONSTRONG);
	}
	...
	if(dataflag)printf("%% PRODUCTIVITY RATIO: %lf   N_MOVES: %lu\n",productivity_ratio,productivity_ratio_count);
...
void print_postscript_sampleDensity(const struct pageinfo_struct *pageinfo, const struct sampleDensity_struct *sd, const struct script_struct *script, const struct graph_struct *graph){
	...
	plotinfo.xa=0;
	plotinfo.xb=(long)sd->Ncol;
	plotinfo.xc=(long)(ceil)((float)(sd->Ncol)/(float)script->Nreplicas);
...
void print_postscript_pmf(unsigned char ligand_number, const struct pageinfo_struct *pageinfo, const struct script_struct *script, const exact_struct *exact, const struct pmf_struct *pmf, const struct graph_struct *graph){
	...
	if(script->coordinate_type==Temperature){
		...
		plotinfo.xc=(long)(ceil)((float)(pmf->Ncol)/(float)script->Nreplicas);
		binlabels=(float *)calloc((pmf->Ncol),sizeof(float));
		if(binlabels==NULL){
			fprintf(stderr,"Error: Memory allocation error for binlabels\n");
			exit(1);
		}
		for(long b=0; b<pmf->Ncol; ++b){
...
void print_postscript_samplingOverlap(long whichData, const struct pageinfo_struct *pageinfo, const struct script_struct *script, struct database_struct *db, const struct nominal_struct *nominal, const struct graph_struct *graph, const struct analysis_option_struct *opt, const struct stats_struct *stats){
	...
	histo=(long **)malloc((script->Nreplicas)*sizeof(long *));
	for(i=0; i<script->Nreplicas; i++){
		histo[i]=(long *)calloc((numBins+1),sizeof(long));
	...
	tot=(long *)calloc((script->Nreplicas),sizeof(long));
	if(tot==NULL){
	...
	for(i=0;i<db->Nrecords;i++){
		for(j=script->Nsamples_per_run*(whichData); j<script->Nsamples_per_run*(whichData+1); j++){
			if(db->record[i]->sequence_number<(unsigned long)(opt->equilFraction*stats->max_sequence_number)){
				continue;
			}
			//fprintf(stderr,"Putting w:%f val:%f in histo[%ld][%ld]\n",record[i]->w,record[i]->generic_data[j],find_bin_from_w(record[i]->w),(long)floor((record[i]->generic_data[j]-min)/binWidth));
			++histo[find_bin_from_w(db->record[i]->w,0,nominal,script)][(long)floor((db->record[i]->generic_data[j]-min)/binWidth)];
	...
	plotinfo.yscalemult=(absmax-absmin)/(float)pageinfo->numvalues;
	plotinfo.xa=0;
	plotinfo.xb=(long)numBins;
	plotinfo.xc=(long)(ceil)((float)(numBins)/(float)script->Nreplicas);
	plotinfo.ya=round_up(absmin/plotinfo.yscalemult);
	plotinfo.yb=round_down(absmax/plotinfo.yscalemult);
	plotinfo.yc=1;
...
void print_postscript_Pupdown(const struct pageinfo_struct *pageinfo,struct detailedBalance_struct *detbal, const struct script_struct *script, const struct graph_struct *graph){
	...
	for(i=0;i<script->Nreplicas-1;i++){
		...
		printf("%% [p(%ld)*p(%ld->%ld)]/[p(%ld)*p(%ld->%ld)] = %f\n",i,i,i+1,i+1,i+1,i,ioverj);
	}
	//offset=minsumf;
...
long read_in_database(const struct analysis_option_struct *opt, const struct script_struct *script, struct database_struct *db, const struct nominal_struct *nominal){
	...
	if(db->Nadditional_data!=script->Nadditional_data){
		fprintf(stderr,"Error: The forcedatabase has %lu additional data types, while the input script specifies %lu\n",db->Nadditional_data,script->Nadditional_data);
		return 1;
	}
	...
	size=(long)db->Nrecords*(long)db->record_size;
	fprintf(stderr,"The database size should be %lld excluding the header\n",size);
	if(size>MAX_DATABASE_SIZE){
		...
		fprintf(stderr,"Nrecords adjusted to %lu\n",db->Nrecords);	
		fprintf(stderr,"%lu bytes of the database will be considered\n",size);
	}
	
	if( (db->unsorted_record=(struct record_struct*)malloc(size))==NULL ){
	...
	fprintf(stderr,"Reading %ld records",db->Nrecords);
	for(i=0;i<db->Nrecords;i++){
	...
	for(i=0;i<db->Nrecords-1;i++){
		if(compare_records(db->record[i],db->record[i+1])==0){
			...
			fprintf(stderr, "\nEliminating duplicate record at position %ld   ",i);
			memmove(db->record+i+1, db->record+i+2, ((long)db->Nrecords-i-2)*sizeof(struct record_struct *));
			i--;
			db->Nrecords--;
		}
	}
	...
	if(opt->sequence_number_limit>=0){
		...
		for(i=0;i<db->Nrecords;i++)
			if(db->record[i]->sequence_number>opt->sequence_number_limit){
				memmove(db->record+i, db->record+i+1, ((long)db->Nrecords-i-1)*sizeof(struct record_struct *));
				i--;
				db->Nrecords--;
				count++;
			}
		fprintf(stderr,"removed %lu records.\n",count);
	}
	...
	for(i=0;i<db->Nrecords-1;i++){
		if(compare_records(db->record[i],db->record[i+1])!=-1){
			...
			fprintf(stderr,"Record 1: replica number: %ld   sequence_number: %lu\n",db->record[i]->replica_number,db->record[i]->sequence_number);
			fprintf(stderr,"Record 2: replica number: %ld   sequence_number: %lu\n",db->record[i+1]->replica_number,db->record[i+1]->sequence_number);
			return 1;
		}
	}
	fprintf(stderr,"done.\n");
...
char get_next_force(float w[2], float force[2], float *rc, long whichRC, unsigned long *time, long *replica_number, unsigned short *sequence_number, const struct database_struct *db){
	static unsigned long recordN=0;
	static unsigned long sampleN=0;
...
void condense_forces(unsigned long N_values_to_average, struct nominal_struct *nominal, struct graph_struct *graph, const struct script_struct *script, const analysis_option_struct *opt, struct stats_struct *stats, const struct database_struct *db){
	...
	for(i=0;i<script->Nreplicas;i++){
		...
		if(opt->verbose)fprintf(stderr,"Adding: Nreplicas: %lu   w: %lf   w2: %lf\n",i,graph[i].w[0],graph[i].w[1]);
	}
	...
	fprintf(stderr,"Number of discrete w positions: %lu\n",script->Nreplicas);
	fprintf(stderr,"max time is: %lu\n",stats->max_time);
	...
	for(i=0;i<script->Nreplicas;i++){
		...
		for(l=0;l<script->Nligands;l++){
			...
			//fprintf(stderr,"Weight sum of graph points at w = %f: %lu\n",graph[i].w[0],graph[i].weight_sum);
			unsigned long max_force_points=0;
			for(j=0;j<N_FORCE_POINTS;j++) 
				if(graph[i].weight_sum[j]>1.0e-5) max_force_points=j;
...
long round_up(float x)
{
	if(x<0.0) return( (long)x );
	else      return( (long)(x+0.999999) );
}

long round_down(float x)
{
	if(x<0.0) return( (long)(x-0.999999) );
	else      return( (long)x );
}
...
// Data must be allocated for sd->b on [Nreplicas+1][sd->Ncol+1]
	...
	for(i=0;i<db->Nrecords;i++){
		for(j=script->Nsamples_per_run*(whichData); j<script->Nsamples_per_run*(whichData+1); j++){
			...
			++sd->b[db->record[i]->replica_number][(long)floor((val-sd->min)/sd->binWidth)];
		}
	}
	for(i=0;i<script->Nreplicas; i++){
...
//Important note: this is not going to be perfect for temperature since it includes all data without kB factor
	...
	for(i=0;i<db->Nrecords;i++){
		for(j=script->Nsamples_per_run*(whichData),k=0; j<script->Nsamples_per_run*(whichData+1); j++,k++){
			...
			pmf->f[(long)floor((val-pmf->min)/pmf->binWidth)]+=db->record[i]->generic_data[k];
			++pmf->n[(long)floor((val-pmf->min)/pmf->binWidth)];
		}
	}
	//Dump max value down to bin below
